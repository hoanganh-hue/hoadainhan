/**
 * Script để cập nhật mô tả chi tiết sản phẩm từ file JSON cho tất cả các thương hiệu
 * 
 * Script này sẽ:
 * 1. Xử lý lần lượt từng thương hiệu để tránh timeout
 * 2. Đọc file JSON mô tả chi tiết từ thư mục của mỗi thương hiệu
 * 3. Tìm sản phẩm khớp trong cơ sở dữ liệu
 * 4. Cập nhật mô tả chi tiết cho sản phẩm
 */
...
// Ánh xạ thư mục thương hiệu với slug thương hiệu trong DB và đường dẫn file JSON
const brandMapping = [
  { folder: 'honor', slug: 'honor', jsonFile: 'honor.json', jsonKey: 'điện thoại Honor' },
  { folder: 'huawei', slug: 'huawei', jsonFile: 'huawei.json', jsonKey: 'điện thoại Huawei' },
  { folder: 'ipone', slug: 'apple', jsonFile: 'motasanpham.json', jsonKey: 'điện thoại Apple' },
  { folder: 'máy xiaomi', slug: 'xiaomi', jsonFile: 'xiaomi.json', jsonKey: 'điện thoại Xiaomi' },
  { folder: 'oppo', slug: 'oppo', jsonFile: 'oppo.json', jsonKey: 'điện thoại Oppo' },
  { folder: 'vivo', slug: 'vivo', jsonFile: 'vivo.json', jsonKey: 'điện thoại Vivo' }
];
...
/**
 * Lấy danh sách sản phẩm theo slug thương hiệu
 * @param {string} brandSlug Slug thương hiệu
...
async function getProductsByBrand(brandSlug) {
  ...
  return await execute(query, [brandSlug]);
}
...
/**
 * Xử lý cập nhật mô tả cho một thương hiệu
 * @param {Object} brandInfo Thông tin thương hiệu
 * @returns {Promise<number>} Số sản phẩm được cập nhật
 */
async function processBrand(brandInfo) {
  console.log(`\n===== Đang xử lý thương hiệu: ${brandInfo.slug.toUpperCase()} =====`);
  
  // Đọc dữ liệu JSON
  const jsonPath = path.join('dien-thoai', brandInfo.folder, brandInfo.jsonFile);
  const jsonData = await readJsonFile(jsonPath);
  
  if (!jsonData) {
    console.error(`Không thể đọc dữ liệu JSON cho thương hiệu: ${brandInfo.folder}`);
    return 0;
  }
  
  // Lấy danh sách điện thoại từ JSON
  const phonesData = jsonData[brandInfo.jsonKey];
  
  if (!phonesData || !Array.isArray(phonesData)) {
    console.error(`Không tìm thấy dữ liệu sản phẩm hợp lệ trong file JSON cho ${brandInfo.folder} với khóa ${brandInfo.jsonKey}`);
    return 0;
  }
  
  console.log(`Đọc được ${phonesData.length} mô tả từ file JSON`);
  
  // Lấy danh sách sản phẩm từ cơ sở dữ liệu
  const products = await getProductsByBrand(brandInfo.slug);
  ...
  console.log(`Đã cập nhật ${updatedCount}/${phonesData.length} mô tả cho thương hiệu ${brandInfo.slug}`);
  return updatedCount;
}
...
async function main() {
  try {
    ...
    // Chạy lần lượt từng thương hiệu để tránh timeout
    for (const brand of brandMapping) {
      const updatedCount = await processBrand(brand);
      totalUpdated += updatedCount;
      
      // Thêm độ trễ nhỏ giữa các lần xử lý để giảm tải
      await new Promise(resolve => setTimeout(resolve, 500));
    }
    
    console.log(`\n==== Tổng kết ====`);
    console.log(`Đã cập nhật tổng cộng ${totalUpdated} sản phẩm cho tất cả thương hiệu.`);
    console.log("\nHoàn tất cập nhật mô tả sản phẩm từ file JSON.");
  } catch (error) {
