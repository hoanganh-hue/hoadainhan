/**
 * Script để cập nhật mô tả chi tiết sản phẩm từ file JSON cho các thương hiệu điện thoại
 * 
 * Script này sẽ:
 * 1. Đọc file JSON mô tả chi tiết từ thư mục của mỗi thương hiệu
 * 2. Tìm sản phẩm khớp trong cơ sở dữ liệu
 * 3. Cập nhật mô tả chi tiết cho sản phẩm
 */
...
// Ánh xạ thư mục thương hiệu với slug thương hiệu trong DB
const brandMapping = {
  'samsung': 'samsung',
  'honor': 'honor',
  'huawei': 'huawei',
...
- Màn hình: ${specs.màn_hình || specs.màn || specs.màn_hình}
- Chip: ${specs.bộ_xử_lý || specs.chip || specs.bộ_xử_lý}
- RAM: ${specs.ram || specs.RAM}
- Bộ nhớ trong: ${specs.lưu_trữ || specs.bộ_nhớ_trong || specs.lưu_trữ}
- Camera: ${specs.camera || specs.Camera}
- Pin: ${specs.pin || specs.Pin}
- Hệ điều hành: ${specs.hệ_điều_hành || specs.hệ_điều_hành}
 ...
 * @param {string} brandFolder Tên thư mục thương hiệu
 * @returns {Promise<void>}
 */
async function updateBrandDescriptions(brandFolder) {
  const brandSlug = brandMapping[brandFolder];
  if (!brandSlug) {
    console.error(`Không tìm thấy ánh xạ cho thư mục: ${brandFolder}`);
    return;
  }
  
  // Xác định đường dẫn file JSON
  let jsonPath;
  if (brandFolder === 'ipone') {
    jsonPath = path.join('dien-thoai', brandFolder, 'motasanpham.json');
  } else if (brandFolder === 'máy xiaomi') {
    jsonPath = path.join('dien-thoai', brandFolder, 'xiaomi.json');
  } else {
    jsonPath = path.join('dien-thoai', brandFolder, `${brandFolder}.json`);
  }
  
  // Đọc file JSON
  const jsonData = await readJsonFile(jsonPath);
  if (!jsonData) {
    console.error(`Không thể đọc dữ liệu JSON cho thương hiệu: ${brandFolder}`);
    return;
  }
  
  // Lấy danh sách điện thoại từ JSON
  const phonesKey = Object.keys(jsonData)[0]; // Lấy key đầu tiên (ví dụ: "điện thoại Samsung")
  const phonesData = jsonData[phonesKey];
  
  // Lấy danh sách sản phẩm từ cơ sở dữ liệu
  const products = await getProductsByBrand(brandSlug);
  console.log(`Tìm thấy ${products.length} sản phẩm cho thương hiệu ${brandSlug}`);
  ...
  console.log(`Đã cập nhật ${updatedCount}/${phonesData.length} mô tả cho thương hiệu ${brandSlug}`);
}
...
async function main() {
  try {
    // Duyệt qua các thư mục thương hiệu
    const brandFolders = Object.keys(brandMapping);
    
    for (const brandFolder of brandFolders) {
      console.log(`\n===== Xử lý thương hiệu: ${brandFolder} =====`);
      await updateBrandDescriptions(brandFolder);
    }
