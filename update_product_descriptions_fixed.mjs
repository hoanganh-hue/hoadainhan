/**
 * Script để cập nhật mô tả chi tiết sản phẩm Samsung từ file JSON
 * 
 * Script này chỉ tập trung vào một thương hiệu (Samsung) để tránh timeout
 */
...
async function main() {
  try {
    // Đọc dữ liệu JSON Samsung
    const jsonPath = path.join('dien-thoai', 'samsung', 'samsung.json');
    const jsonData = await readJsonFile(jsonPath);
    
    if (!jsonData) {
      console.error('Không thể đọc dữ liệu JSON Samsung');
      return;
    }
    
    // Lấy danh sách điện thoại Samsung từ JSON
    const phonesKey = Object.keys(jsonData)[0]; // "điện thoại Samsung"
    const phonesData = jsonData[phonesKey];
    
    console.log(`Đọc được ${phonesData.length} mô tả Samsung từ file JSON`);
    
    // Lấy danh sách sản phẩm Samsung từ cơ sở dữ liệu
    const products = await getSamsungProducts();
    console.log(`Tìm thấy ${products.length} sản phẩm Samsung trong cơ sở dữ liệu`);
    ...
    console.log(`Đã cập nhật ${updatedCount}/${phonesData.length} mô tả cho thương hiệu Samsung`);
    console.log("\nHoàn tất cập nhật mô tả sản phẩm Samsung từ file JSON.");
  } catch (error) {
